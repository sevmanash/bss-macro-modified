...
def macro(status, log, haste, updateGUI):
    ...
    while True:
        ...
        if macro.setdat["Auto_Field_Boost"]:
            if macro.hasAFBRespawned("AFB_rebuff", macro.setdat["AFB_rebuff"]*60, applyMobRespawnBonus=False):
                runTask(macro.AFB)
        ...
        for k, _ in macroModule.fieldBoosterData.items():
            #check if the cooldown is up
            if macro.setdat[k] and macro.hasAFBRespawned(k, macro.collectCooldowns[k]) and macro.hasAFBRespawned("last_booster", macro.setdat["boost_seperate"]*60):
        ...
        for field in boostedGatherFields:
            ...
            while time.time() - st < 15*60:
                #auto field boost
                if macro.setdat["Auto_Field_Boost"]:
                    if macro.hasAFBRespawned("AFB_glitter_cd", macro.setdat["AFB_rebuff"]*60+60) or macro.hasAFBRespawned("AFB_dice_cd", macro.setdat["AFB_rebuff"]*60-60):
                        runTask(macro.AFB)
